...
def debug_timestamps():
    """Debug: shows ALL active posts with timestamps"""
    now = datetime.utcnow()
    now_ts = _utc_epoch(now)
    cutoff = 24 * 3600
    posts = AudioPost.query.filter(AudioPost.is_active == True).order_by(AudioPost.created_at.asc()).all()
    return jsonify({
        "now_utc": str(now),
        "now_epoch": now_ts,
        "cutoff_seconds": cutoff,
        "total_active": len(posts),
        "posts": [{
            "id": p.id[:8],
...
def toggle_bookmark(post_id):
    ...
    if existing:
        ...
        bm = PostBookmark(user_id=current_user.id, post_id=post.id)
        db.session.add(bm)
        db.session.commit()
...
@login_required
def get_bookmarks():
    bms = PostBookmark.query.filter_by(user_id=current_user.id).order_by(
        PostBookmark.created_at.desc()
    ).all()
    post_ids = [bm.post_id for bm in bms]
    posts = AudioPost.query.filter(
        AudioPost.id.in_(post_ids), AudioPost.is_active == True
    ).all()
    posts_by_id = {p.id: p for p in posts}
    return jsonify([posts_by_id[pid].to_dict(current_user.id) for pid in post_ids if pid in posts_by_id])
...
def is_bookmarked(post_id):
    ...
    return jsonify({"bookmarked": bm is not None})
